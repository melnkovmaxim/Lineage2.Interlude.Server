...
{
	private FastTable<ServerData> _servers;
	...
	{
		_servers = new FastTable<ServerData>();
		_lastServer = client.getLastServer();
		for (GameServerInfo gsi : GameServerTable.getInstance().getRegisteredGameServers().values())
		{
			if (gsi.getStatus() == ServerStatus.STATUS_DOWN)
				continue;

			int nextId;
			if (Config.PROXYS.containsKey(gsi.getId()) && gsi.getStatus() != ServerStatus.STATUS_GM_ONLY)
			{
				nextId = gsi.getId() + 1;
				for (String ip : Config.PROXYS.get(gsi.getId()))
				{
					addServer(ip, gsi.getPort(), gsi.isPvp(), gsi.isTestServer(), gsi.getCurrentPlayerCount(), gsi.getMaxPlayers(), gsi.isShowingBrackets(), gsi.isShowingClock(), 1, nextId);
					nextId++;
				}
			}
			...
			if (gsi.getStatus() == ServerStatus.STATUS_GM_ONLY && client.getAccessLevel() >= Config.GM_MIN)
			{
				...
				if (Config.PROXYS.containsKey(gsi.getId()))
				{
					nextId = gsi.getId() + 1;
					for (String ip : Config.PROXYS.get(gsi.getId()))
					{
			...
			else if (gsi.getStatus() != ServerStatus.STATUS_GM_ONLY && Config.SHOW_MAIN_IP)
			{

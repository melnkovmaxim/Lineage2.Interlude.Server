package events.q9111_SpecialBoss;
...
public class q9111_SpecialBoss extends QuestJython {

    //рб
    private final static int BOSS_ID = 50031;
    // локация
    private final static int[] BOSS_LOC = {88726, 258110, -10378};

    //  чтобы был анонс (текст вывод в конфиг)
    private final static String ANNOUNCE_TEXT = "Через 10 минут появится РБ Death Lord (Дроп - любая донат вещь, шанс 100%), телепортироваться к нему можно через нпц Беном.";
    private final static String ANNOUNCE_SPAWN = "РБ Death Lord (Дроп - любая донат вещь, шанс 100%) появился, телепортироваться к нему можно можно через нпц Беном.";

    // который будет появляться каждый день в 20:00 по мск (т.е., чтобы запускался как осада по времени компа)
    private final static String BOSS_SPAWN_TIME = "19:30";

    //на сколько времени он появляется, минуты
    private final static int DURATION = 80;

     //перед появленим анонс идет, за сколько минут?
    private final static int ANNOUNCE = 10;
    ...
    public static void main(String... arguments) {
        new q9111_SpecialBoss(9111, "q9111_SpecialBoss", "SpecialBoss");
    }

    public q9111_SpecialBoss(int questId, String name, String descr) {

...
class Quest (JQuest) :
   ...
   if event == "31521-02.htm" :
      if st.getPlayer().getLevel() < 73 : 
       ...
       if st.getQuestItemsCount(FOOD) == 0 :
           ...
           spawnId = st.addSpawn(BUMPALUMP,158240,-121536,-2253,40240,False,0)
           st.takeItems(FOOD,1)
           npc.setBusy(True)
           st.set("cond","2")
 ...
 def onKill(self,npc,player,isPet):
    ...
    if npcId == BUMPALUMP :
        FindTemplate(TABLE).setBusy(False)
        #self.getQuestTimer("Icicle Emperor Bumbalump has despawned",npc,None).cancel() # FIXME: ? 

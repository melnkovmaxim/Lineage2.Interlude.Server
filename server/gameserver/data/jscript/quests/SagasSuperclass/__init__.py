...
from ru.agecold.gameserver.datatables import SpawnTable
from ru.agecold.gameserver.model import L2Spawn
from ru.agecold.gameserver.ai import CtrlIntention
from ru.agecold.gameserver.ai import CtrlEvent
from ru.agecold.gameserver.network.serverpackets import MagicSkillUser
...
class Quest (JQuest) :
 ...
 def AutoChat(self, npc,text) :
    ...
    if chars != None:
       for pc in chars :
          pc.broadcastPacket(CreatureSay(npc.getObjectId(),0,npc.getName(),text))
 ...
 def onAdvEvent (self,event,npc,player) :
   ...
   if event in ["0-011.htm", "0-012.htm", "0-013.htm", "0-014.htm", "0-015.htm"]:
       ...
       st.giveItems(self.Items[9],1)
       st.set("cond","18")
       self.AutoChat(npc,self.Text[13].replace('PLAYERNAME',player.getName()))
       st.set("Quest0","0")
       self.cancelQuestTimer("Mob_2 has despawned",npc,player)
       st.playSound("ItemSound.quest_middle")
       ...
       if npc.getKnownList().knowsObject(Mob_2) :
           ...
           self.cancelQuestTimer("Mob_3 Timer 1",npc,player)
       return
 ...
 def onTalk (self,npc,player):
    ...
    if st :
      npcId = npc.getNpcId()
      cond = st.getInt("cond")
      if st.getState() == 'Complteted' and npcId == self.NPC[0] :
          ...
          if cond == 0 :
              ...
              if npcId == self.NPC[1] and st.getQuestItemsCount(self.Items[0]) :
                      htmltext = "1-02.htm"
                      if self.Items[11] == 0 or st.getQuestItemsCount(self.Items[11]) :
                          htmltext = "1-03.htm"
          elif cond == 4 :
              if npcId == self.NPC[1] :
                  htmltext = "1-04.htm"
 ...
 def onAttack (self, npc, player, damage, isPet):
    ...
    if npcId == self.Mob[2] and st == st2 and cond == 17 :
        ...
        if st.getInt("Quest0") > 15 :
            ...
            self.cancelQuestTimer("Mob_3 has despawned",npc,player)
            st.set("Tab","1")
            self.DeleteSpawn(st,npc.getObjectId())
        ...
        if st != st2 or (st == st2 and player.isInParty()) :
            ...
            self.cancelQuestTimer("Archon Hellisha has despawned",npc,player)
            st2.set("spawned","0")
        ...
        if npcId == self.Mob[0] and cond == 8 :
            if not player.isInParty():
                if st == st2 :
                    ...
                    st.set("cond","9")
                    st.playSound("ItemSound.quest_middle")
            self.cancelQuestTimer("Mob_1 has despawned",npc,player)
            st2.set("spawned","0")
            self.DeleteSpawn(st2,npc.getObjectId())
        elif npcId == self.Mob[1] and cond == 15 :
            if not player.isInParty():

...
from ru.agecold.gameserver.model.actor.instance import L2NpcInstance
from ru.agecold.gameserver.model                import L2CharPosition
...
class retreat_onattack(JQuest) :
    ...
    def __init__(self,id,name,descr):
        self.MobSpawns ={
                20002: {'HP': 100, 'chance': 100}, # Rabbit
                20432: {'HP': 100, 'chance': 100}, # Elpy
                20058: {'HP': 50, 'chance': 10} # Ol Mahum Guard
                }
        # made a second dictionary for the texts
        self.MobTexts ={
                20058: ["Я вернусь", "Вы более сильны чем я ожидал"] # I don't recall the retail text they say, so I made custom ones to test it
                }
    ...
    def onAttack(self,npc,player,damage,isPet) :
        ...
        if self.MobSpawns.has_key(npcId) :
            if npc.getStatus().getCurrentHp() <= npc.getMaxHp() * self.MobSpawns[npcId]['HP'] / 100 and Rnd.get(100) < self.MobSpawns[npcId]['chance'] :
                ...
                self.startQuestTimer("Retreat", 5000, npc, player)
        return
